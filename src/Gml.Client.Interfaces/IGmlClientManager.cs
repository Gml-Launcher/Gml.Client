...
public interface IGmlClientManager : IDisposable
{
    ...
    Task<(ILauncherUser User, string Message, IEnumerable<string> Details)> Auth(string login, string password, string hwid);
    Task<(ILauncherUser User, string Message, IEnumerable<string> Details)> Auth(string accessToken);
